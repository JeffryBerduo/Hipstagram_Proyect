...
const createPost = async (req, res) => {
  ...
  if (description.length > 128) {
    return res.status(400).json({ message: "La descripción no puede superar 128 caracteres" });
  }
...
const deletePost = async (req, res) => {
  ...
  try {
    ...
    if (check.rows.length === 0) {
      await client.query("ROLLBACK");
      return res.status(404).json({ message: "Publicación no encontrada o no te pertenece" });
    }

    await client.query(`DELETE FROM publicaciones_hashtags WHERE post_id = $1`, [id]);
...
const adminDeletePost = async (req, res) => {
  ...
  try {
    ...
    const del = await client.query(
      `DELETE FROM publicaciones WHERE id = $1 RETURNING id`, [id]
    );
